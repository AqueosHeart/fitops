...
function desktopHeader(width = 1440) {
  ...
  const nav = textNode('Inicio     Actividades     Horario     Servicios     Instalaciones     Tarifas     Equipo     Contacto', 10, 'medium', C.ink);
  nav.x = 320; nav.y = 31; header.appendChild(nav);
  const access = textNode('Acceder', 11, 'bold', C.blue); access.x = 1328; access.y = 31; header.appendChild(access);
  return header;
}
...
function mobileHeader() {
  ...
  const action = textNode('Horario  Menu', 10, 'medium', C.blue); action.x = 282; action.y = 24; header.appendChild(action);
  return header;
}
...
async function buildDesktopScreens(page) {
  ...
  let base = createDesktopBase('D00 · Landing / Complete', 'Entrena con propósito. Vive con más energía.', 'Una experiencia pública ficticia de Practice Athletic Club, desde descubrir el centro hasta reservar tu próxima sesión.');
  ...
  const heroCopy = auto('VERTICAL', 16, 0); heroCopy.resize(720, 100); heroCopy.counterAxisSizingMode = 'FIXED'; heroCopy.primaryAxisSizingMode = 'AUTO'; heroCopy.appendChild(await kitBadge('PRACTICE ATHLETIC CLUB')); heroCopy.appendChild(textNode('Fuerza, ritmo y recuperación para tu vida real.', 38, 'bold', C.ink, 720)); heroCopy.appendChild(textNode('Encuentra actividades, conoce el equipo y reserva sesiones desde una experiencia clara y ficticia.', 15, 'regular', C.muted, 660)); const heroActions = auto('HORIZONTAL', 12, 0); heroActions.appendChild(await kitButton('Ver actividades', 'secondary', 180)); heroActions.appendChild(await kitButton('Ver horario', 'primary', 160)); heroCopy.appendChild(heroActions);
  const heroPanel = auto('VERTICAL', 12, 24); heroPanel.resize(500, 250); heroPanel.counterAxisSizingMode = 'FIXED'; heroPanel.fills = [paint(C.blueSoft)]; heroPanel.cornerRadius = 6; heroPanel.appendChild(textNode('TU SEMANA EN PRACTICE', 11, 'bold', C.blue, 452)); heroPanel.appendChild(textNode('18 sesiones guiadas', 26, 'bold', C.ink, 452)); heroPanel.appendChild(textNode('Fuerza, movilidad, intervalos y espacios de entrenamiento libre.', 13, 'regular', C.muted, 452)); heroPanel.appendChild(await kitButton('Consultar horario', 'secondary', 180)); hero.appendChild(heroCopy); hero.appendChild(heroPanel); base.content.appendChild(hero);
  const stats = box(1312, 78, C.dark, null, 0); const statsText = textNode('ABIERTO TODOS LOS DÍAS        ·        ACTIVIDADES GUIADAS        ·        RESERVA DESDE LA APP        ·        DATOS DEMO', 12, 'bold', C.white, 1260, 'CENTER'); statsText.x = 26; statsText.y = 29; stats.appendChild(statsText); base.content.appendChild(stats);
  ...
  base.content.appendChild(sectionHeading('Actividades', 'Una rutina que no se queda quieta.', 'Explora modalidades para fuerza, ritmo, movilidad y entrenamiento funcional.'));
  const activityRow = auto('HORIZONTAL', 16, 0); [['Fuerza', 'Técnica, progresión y carga.'], ['Pace', 'Intervalos para capacidad y energía.'], ['Reset', 'Movilidad, control y recuperación.'], ['Open Floor', 'Entrena a tu propio ritmo.']].forEach(([title, body]) => { const card = auto('VERTICAL', 10, 18); card.resize(316, 180); card.counterAxisSizingMode = 'FIXED'; card.fills = [paint(C.white)]; card.strokes = [paint(C.border)]; card.cornerRadius = 6; const placeholder = box(280, 62, C.canvas, C.line, 4); const label = textNode('ACTIVIDAD', 10, 'bold', C.muted); label.x = 12; label.y = 24; placeholder.appendChild(label); card.appendChild(placeholder); card.appendChild(textNode(title, 18, 'bold', C.ink, 280)); card.appendChild(textNode(body, 12, 'regular', C.muted, 280)); activityRow.appendChild(card); }); base.content.appendChild(activityRow);

  base.content.appendChild(sectionHeading('Servicios', 'Te acompañamos antes, durante y después.', 'La experiencia combina evaluación inicial, guía de entrenamiento, seguimiento y recuperación.'));
  const serviceRow = auto('HORIZONTAL', 16, 0); [['Valoración inicial', 'Objetivos y punto de partida.'], ['Plan de entrenamiento', 'Una ruta visible para tu progreso.'], ['Recuperación', 'Espacios y hábitos para bajar revoluciones.']].forEach(([title, body]) => { const card = auto('VERTICAL', 8, 20); card.resize(426, 142); card.counterAxisSizingMode = 'FIXED'; card.fills = [paint(C.blueSoft)]; card.cornerRadius = 6; card.appendChild(textNode(title, 16, 'bold', C.ink, 386)); card.appendChild(textNode(body, 12, 'regular', C.muted, 386)); card.appendChild(textNode('Conocer servicio →', 11, 'bold', C.blue, 386)); serviceRow.appendChild(card); }); base.content.appendChild(serviceRow);

  base.content.appendChild(sectionHeading('Instalaciones', 'Espacios pensados para entrenar y recuperarte.', 'Una vista rápida de las zonas que harían parte de este club ficticio.'));
  const facilityGrid = auto('HORIZONTAL', 16, 0); [['Sala de fuerza', 420], ['Zona de ritmo', 280], ['Movilidad', 280], ['Recuperación', 280]].forEach(([name, width]) => { const card = box(width, 214, C.canvas, C.line, 6); const placeholder = textNode(`IMAGEN\n${name.toUpperCase()}`, 12, 'bold', C.muted, width - 32, 'CENTER'); placeholder.x = 16; placeholder.y = 88; card.appendChild(placeholder); facilityGrid.appendChild(card); }); base.content.appendChild(facilityGrid);

  base.content.appendChild(sectionHeading('Tarifas', 'Elige la forma de entrenar que encaja contigo.', 'Planes ficticios, mostrados para definir la jerarquía y el comparador del producto.'));
  const priceRow = auto('HORIZONTAL', 16, 0); const plans = [['Base', 'Acceso esencial', '$39 / mes'], ['Complete', 'Sesiones guiadas', '$59 / mes'], ['Training+', 'Acompañamiento total', '$79 / mes']];
  for (const [index, [name, detail, price]] of plans.entries()) { const card = auto('VERTICAL', 10, 20); card.resize(426, 245); card.counterAxisSizingMode = 'FIXED'; card.fills = [paint(index === 1 ? C.dark : C.white)]; card.strokes = [paint(index === 1 ? C.dark : C.border)]; card.cornerRadius = 6; const ink = index === 1 ? C.white : C.ink; card.appendChild(textNode(name, 18, 'bold', ink, 386)); card.appendChild(textNode(detail, 12, 'regular', index === 1 ? C.canvas : C.muted, 386)); card.appendChild(textNode(price, 28, 'bold', ink, 386)); card.appendChild(textNode('✓ Horario y disponibilidad\n✓ Perfil y reservas\n✓ Datos ficticios', 11, 'regular', index === 1 ? C.canvas : C.muted, 386)); card.appendChild(await kitButton('Ver tarifa', index === 1 ? 'primary' : 'secondary', 150)); priceRow.appendChild(card); }
  base.content.appendChild(priceRow);

  base.content.appendChild(sectionHeading('Equipo', 'El acompañamiento es parte del entrenamiento.', 'Perfiles ficticios para definir cómo se presenta el equipo y sus especialidades.'));
  const teamRow = auto('HORIZONTAL', 16, 0); [['María Torres', 'Fuerza'], ['Daniel Ríos', 'Pace'], ['Sofía Méndez', 'Movilidad'], ['Lucas Vega', 'Atención al miembro']].forEach(([name, role]) => { const card = auto('VERTICAL', 8, 16); card.resize(316, 174); card.counterAxisSizingMode = 'FIXED'; card.fills = [paint(C.white)]; card.strokes = [paint(C.border)]; card.cornerRadius = 6; const avatar = box(72, 72, C.canvas, C.line, 999); const avatarText = textNode('FOTO', 10, 'bold', C.muted); avatarText.x = 18; avatarText.y = 28; avatar.appendChild(avatarText); card.appendChild(avatar); card.appendChild(textNode(name, 15, 'bold', C.ink, 280)); card.appendChild(textNode(role, 11, 'regular', C.muted, 280)); teamRow.appendChild(card); }); base.content.appendChild(teamRow);

  base.content.appendChild(sectionHeading('Contacto', 'Conoce el centro antes de empezar.', 'Información ficticia para horarios, ubicación, accesibilidad y primer contacto.'));
  const contactRow = auto('HORIZONTAL', 16, 0); const map = box(650, 196, C.canvas, C.line, 6); const mapText = textNode('MAPA / UBICACIÓN\nDATOS FICTICIOS', 12, 'bold', C.muted, 610, 'CENTER'); mapText.x = 20; mapText.y = 82; map.appendChild(mapText); const contact = auto('VERTICAL', 8, 20); contact.resize(646, 196); contact.counterAxisSizingMode = 'FIXED'; contact.fills = [paint(C.white)]; contact.strokes = [paint(C.border)]; contact.cornerRadius = 6; contact.appendChild(textNode('Practice Athletic Club', 17, 'bold', C.ink, 606)); contact.appendChild(textNode('Av. del Entrenamiento 100 · Ciudad demo\nLun–Dom · 06:00–22:00\ncontacto@practice.example.invalid', 12, 'regular', C.muted, 606)); contact.appendChild(await kitButton('Contactar al centro', 'secondary', 190)); contactRow.appendChild(map); contactRow.appendChild(contact); base.content.appendChild(contactRow);

  const finalCta = auto('VERTICAL', 12, 28); finalCta.resize(1312, 204); finalCta.counterAxisSizingMode = 'FIXED'; finalCta.fills = [paint(C.blueSoft)]; finalCta.cornerRadius = 8; finalCta.appendChild(textNode('Ya tienes el plan. Falta tu primera sesión.', 27, 'bold', C.ink, 1256)); finalCta.appendChild(textNode('Descubre actividades, consulta horarios y elige el siguiente paso.', 13, 'regular', C.muted, 1256)); finalCta.appendChild(await kitButton('Empezar ahora', 'primary', 170)); base.content.appendChild(finalCta);
  const footer = box(1312, 260, C.dark, null, 0); const footerTitle = textNode('PRACTICE ATHLETIC CLUB', 17, 'bold', C.white); footerTitle.x = 28; footerTitle.y = 28; footer.appendChild(footerTitle); const footerNav = textNode('Explora\nInicio\nActividades\nServicios\nInstalaciones\nTarifas\n\nClub\nEquipo\nContacto\nHorario\n\nLegal\nPrivacidad\nTérminos\nAviso de responsabilidad', 12, 'regular', C.canvas, 720); footerNav.x = 28; footerNav.y = 68; footer.appendChild(footerNav); const footerNote = textNode('Proyecto de portafolio ficticio. Sin datos reales ni producción.\n© Practice Athletic Club', 11, 'regular', C.muted, 430); footerNote.x = 840; footerNote.y = 184; footer.appendChild(footerNote); base.content.appendChild(footer);
...
async function buildMobileScreens(page) {
  ...
  let base = createMobileBase('M00 · Landing / Android Complete', 'Practice Athletic Club', 'Una experiencia de entrenamiento ficticia, desde descubrir hasta reservar.'); screens.landing = base.screen; base.screen.resize(390, 5000); base.bottom.y = 4936; const mobileHero = auto('VERTICAL', 12, 20); mobileHero.resize(358, 280); mobileHero.counterAxisSizingMode = 'FIXED'; mobileHero.fills = [paint(C.white)]; mobileHero.strokes = [paint(C.border)]; mobileHero.cornerRadius = 8; mobileHero.appendChild(await kitBadge('PRACTICE ATHLETIC CLUB')); mobileHero.appendChild(textNode('Entrena con propósito.', 30, 'bold', C.ink, 318)); mobileHero.appendChild(textNode('Fuerza, ritmo y recuperación para una vida real.', 13, 'regular', C.muted, 318)); const mobileHeroActions = auto('HORIZONTAL', 10, 0); mobileHeroActions.appendChild(await kitButton('Actividades', 'secondary', 150)); mobileHeroActions.appendChild(await kitButton('Horario', 'primary', 150)); mobileHero.appendChild(mobileHeroActions); base.content.appendChild(mobileHero); const mobileSection = (eyebrow, title, body) => { const section = auto('VERTICAL', 5, 0); section.appendChild(textNode(eyebrow.toUpperCase(), 10, 'bold', C.blue, 358)); section.appendChild(textNode(title, 22, 'bold', C.ink, 358)); section.appendChild(textNode(body, 12, 'regular', C.muted, 358)); return section; }; base.content.appendChild(mobileSection('Actividades', 'Una rutina que se mueve contigo.', 'Fuerza, pace, reset y open floor.')); for (const program of ['Fuerza · Técnica y progresión', 'Pace · Capacidad y energía', 'Reset · Movilidad y control', 'Open Floor · A tu ritmo']) { const card = box(358, 56, C.white, C.border, 5); const label = textNode(program, 12, 'medium', C.ink); label.x = 14; label.y = 20; card.appendChild(label); base.content.appendChild(card); } base.content.appendChild(mobileSection('Servicios', 'Te guiamos y te cuidamos.', 'Valoración, plan de entrenamiento y recuperación.')); for (const service of ['Valoración inicial', 'Plan de entrenamiento', 'Recuperación']) { const card = box(358, 54, C.blueSoft, null, 5); const label = textNode(`${service}  →`, 12, 'bold', C.ink); label.x = 14; label.y = 19; card.appendChild(label); base.content.appendChild(card); } base.content.appendChild(mobileSection('Instalaciones', 'Espacios que suman a tu entrenamiento.', 'Salas ficticias para fuerza, ritmo, movilidad y recuperación.')); const mobileFacilities = box(358, 180, C.canvas, C.line, 6); const facilityLabel = textNode('GALERÍA DE INSTALACIONES\nIMÁGENES DE REFERENCIA', 11, 'bold', C.muted, 318, 'CENTER'); facilityLabel.x = 20; facilityLabel.y = 76; mobileFacilities.appendChild(facilityLabel); base.content.appendChild(mobileFacilities); base.content.appendChild(mobileSection('Tarifas', 'Elige tu forma de entrenar.', 'Planes ficticios para definir la comparación.')); for (const plan of ['Base · $39 / mes', 'Complete · $59 / mes', 'Training+ · $79 / mes']) { const card = box(358, 66, C.white, C.border, 5); const label = textNode(plan, 14, 'bold', C.ink); label.x = 14; label.y = 15; card.appendChild(label); const sub = textNode('Ver beneficios →', 11, 'medium', C.blue); sub.x = 14; sub.y = 39; card.appendChild(sub); base.content.appendChild(card); } base.content.appendChild(mobileSection('Equipo', 'No entrenas solo.', 'Conoce perfiles y especialidades.')); for (const person of ['María · Fuerza', 'Daniel · Pace', 'Sofía · Movilidad']) { const card = box(358, 52, C.white, C.border, 5); const label = textNode(person, 12, 'medium', C.ink); label.x = 14; label.y = 18; card.appendChild(label); base.content.appendChild(card); } base.content.appendChild(mobileSection('Contacto', 'Visita el centro.', 'Ubicación, horarios y primer contacto.')); const mobileContact = auto('VERTICAL', 8, 16); mobileContact.resize(358, 170); mobileContact.counterAxisSizingMode = 'FIXED'; mobileContact.fills = [paint(C.white)]; mobileContact.strokes = [paint(C.border)]; mobileContact.cornerRadius = 6; mobileContact.appendChild(textNode('Practice Athletic Club', 16, 'bold', C.ink, 326)); mobileContact.appendChild(textNode('Ciudad demo · Lun–Dom 06:00–22:00\ncontacto@practice.example.invalid', 11, 'regular', C.muted, 326)); mobileContact.appendChild(await kitButton('Contactar', 'secondary', 150)); base.content.appendChild(mobileContact); const mobileCta = auto('VERTICAL', 10, 18); mobileCta.resize(358, 154); mobileCta.counterAxisSizingMode = 'FIXED'; mobileCta.fills = [paint(C.blueSoft)]; mobileCta.cornerRadius = 6; mobileCta.appendChild(textNode('Tu primera sesión empieza aquí.', 19, 'bold', C.ink, 322)); mobileCta.appendChild(await kitButton('Empezar ahora', 'primary', 180)); base.content.appendChild(mobileCta); const mobileFooter = box(358, 190, C.dark, null, 0); const mobileFooterText = textNode('PRACTICE ATHLETIC CLUB\n\nInicio · Actividades · Horario\nServicios · Instalaciones · Tarifas\nEquipo · Contacto · Privacidad · Términos\n\nProyecto ficticio de portafolio', 11, 'regular', C.canvas, 318); mobileFooterText.x = 20; mobileFooterText.y = 20; mobileFooter.appendChild(mobileFooterText); base.content.appendChild(mobileFooter);
...
async function buildFitOpsWireframes() {
  ...
  const modules = [
    ['01 Landing', 'Public entry point with a clear Horario route; the first schedule state cascades directly below the landing state.', [desktop.landing, desktop.schedule], [mobile.landing, mobile.schedule]],
    ['02 Schedule', 'Inspect session details and recover safely from availability or domain failures.', [desktop.details, desktop.failures], [mobile.details, mobile.failures]],
  ...
  progress('Linking landing access to the first schedule state…');
  await linkLandingToSchedule(desktop.landing, desktop.schedule);
  await linkLandingToSchedule(mobile.landing, mobile.schedule);
