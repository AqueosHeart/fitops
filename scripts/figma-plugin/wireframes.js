...
function desktopHeader(width = 1440) {
  ...
  const nav = textNode('Inicio     Actividades     Servicios     Instalaciones     Tarifas     Equipo     Contacto', 11, 'medium', C.ink);
  nav.x = 350; nav.y = 31; header.appendChild(nav);
...
async function buildDesktopScreens(page) {
  ...
  const heroCopy = auto('VERTICAL', 16, 0); heroCopy.resize(720, 100); heroCopy.counterAxisSizingMode = 'FIXED'; heroCopy.primaryAxisSizingMode = 'AUTO'; heroCopy.appendChild(await kitBadge('PRACTICE ATHLETIC CLUB')); heroCopy.appendChild(textNode('Fuerza, ritmo y recuperación para tu vida real.', 38, 'bold', C.ink, 720)); heroCopy.appendChild(textNode('Encuentra actividades, conoce el equipo y reserva sesiones desde una experiencia clara y ficticia.', 15, 'regular', C.muted, 660)); heroCopy.appendChild(await kitButton('Ver actividades', 'primary', 180));
