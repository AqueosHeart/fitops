...
- The pending Figma generator targets nine module pages with 15 desktop and 13 Android 390 px states cascading vertically, uses fictional data, reuses local WebbyFrames buttons and badges when available, limits prototype reactions to valid top-level Landing-to-Schedule navigation on the same page, and must not be treated as brand approval.
...
- Landing now exposes the required schedule path through a `Horario` nav entry and CTA. The first desktop and Android schedule states cascade under Landing on the same Figma page and receive its prototype navigation; Schedule detail and failure states remain on the dedicated Schedule page.
